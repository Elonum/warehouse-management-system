...
func (s *InventoryStatusService) GetByID(ctx context.Context, statusID int) (*dto.InventoryStatusResponse, error) {
	status, err := s.repo.GetByID(ctx, statusID)
	if err != nil {
		log.Error().Err(err).Int("statusId", statusID).Msg("Failed to get inventory status by ID")
		return nil, err
	}
...
func (s *InventoryStatusService) Create(ctx context.Context, req dto.InventoryStatusCreateRequest) (*dto.InventoryStatusResponse, error) {
	...
	log.Info().Int("statusId", status.InventoryStatusID).Str("name", status.Name).Msg("Inventory status created successfully")
	return &dto.InventoryStatusResponse{
		InventoryStatusID: status.InventoryStatusID,
		Name:              status.Name,
	}, nil
...
func (s *InventoryStatusService) Update(ctx context.Context, statusID int, req dto.InventoryStatusUpdateRequest) (*dto.InventoryStatusResponse, error) {
	...
	log.Info().Int("statusId", statusID).Msg("Inventory status updated successfully")
	return &dto.InventoryStatusResponse{
		InventoryStatusID: status.InventoryStatusID,
		Name:              status.Name,
	}, nil
...
func (s *InventoryStatusService) Delete(ctx context.Context, statusID int) error {
